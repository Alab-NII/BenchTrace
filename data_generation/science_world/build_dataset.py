"""
build_dataset.py  —  Build the ROGUE snapshot dataset from ScienceWorld AI annotations.

Each snapshot file becomes a separate dataset entry with a unique ID:
  <original_id>_<model_clean>
e.g. scienceworld_boil_var008_ep00_gpt-4.1
...
Usage:
    conda run -n Fraud python build_dataset.py [--output_dir ../../final_dataset/scienceworld]
"""
...
ANN_FILE = BASE / "output" / "scienceworld_all_ai_annotations.json"
...
def build_items(ep: dict) -> list[dict]:
    ...
    # Normalize all where fields in-place before any IoU computation.
...
def main():
    ...
    parser.add_argument("--output_dir", default="../../final_dataset/scienceworld")
    ...
    for (path, snap), ann in zip(failed_snaps, ann_entries):
        ...
        task_name   = snap.get("task_name", "unknown")
    ...
    for task_name, episodes in sorted(by_task.items()):
        ...
        print(f"  {task_name}: {len(episodes)} episodes (failed={failed_n}, won={won_n}) → {out_path}")
