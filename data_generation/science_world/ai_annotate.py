...
AI-based annotation of ScienceWorld episodes using Claude + Gemini.
Reads episode snapshots from ScienceWorld/output/ and annotates failures
using the scienceworld.md prompt. Draft annotation is empty (no rule-based
classifier for ScienceWorld yet).
...
def load_system_prompt() -> str:
    path = PROMPTS_DIR / "scienceworld.md"
    if not path.exists():
...
def load_snapshots(
    ...
    for path in sorted(snapshots_dir.rglob("*.json")):
        ...
        if snap.get("won", True):
            continue
        if task_name and snap.get("task_name") != task_name:
            continue
...
# Limit Gemini to 1 concurrent request to avoid 429s on long ScienceWorld trajectories.
...
def run(args):
    ...
    tag_parts = [x for x in [args.task_name, args.framework, args.model] if x]
    tag = "_".join(tag_parts) if tag_parts else "all"
    output_path = output_dir / f"scienceworld_{tag}_ai_annotations.json"
...
def parse_args():
    ...
    parser.add_argument("--task_name", default=None,
                        help="Filter by task_name, e.g. chemistry-mix (default: all)")
    parser.add_argument("--framework", default=None,
                        help="Filter by framework, e.g. reflexion, evotest (default: all)")
