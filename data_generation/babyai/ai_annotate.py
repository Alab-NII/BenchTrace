...
AI-based annotation of BabyAI episodes using Claude + Gemini.
Reads episode snapshots from BabyAI/output/ and annotates failures
using the babyai.md prompt. Draft annotation is empty (no rule-based
classifier for BabyAI yet).
...
def load_snapshots(
    ...
    Optionally filter by env_id, framework, and model.
    """
    snapshots = []
    for path in sorted(snapshots_dir.rglob("*.json")):
        ...
        if snap.get("won", True):
            continue
        if env_id and snap.get("env_id") != env_id:
            continue
...
def build_user_prompt(snapshot: dict) -> str:
    # Two snapshot schemas coexist: older ones use task_type, newer ones use task_desc.
    # Mission text is always available from the obs field as a fallback.
    snapshot_for_prompt = {
...
def run(args):
    ...
    tag_parts = [x for x in [args.env_id, args.framework, args.model] if x]
    tag = "_".join(tag_parts) if tag_parts else "all"
    output_path = output_dir / f"babyai_{tag}_ai_annotations.json"
...
def parse_args():
    ...
    parser.add_argument("--snapshots_dir", default=str(SNAPSHOTS_DIR),
                        help="Path to BabyAI output directory (default: BabyAI/output)")
    parser.add_argument("--output_dir", default="output",
                        help="Directory to save AI annotation results")
    parser.add_argument("--env_id", default=None,
                        help="Filter by env_id, e.g. BabyAI-Open-v0 (default: all)")
