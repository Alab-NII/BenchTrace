...
AI-based annotation of BundledWebShopping episodes using Claude + Gemini.
Reads episode snapshots from BundledWebShopping/output/ and annotates failures
using the bundled_web_shopping.md prompt. Draft annotation is empty (no rule-based
classifier for BundledWebShopping yet).
...
def load_system_prompt() -> str:
    path = PROMPTS_DIR / "bundled_web_shopping.md"
    if not path.exists():
...
def run(args):
    ...
    tag_parts = [x for x in [args.category, args.framework, args.model] if x]
    tag = "_".join(tag_parts) if tag_parts else "all"
    output_path = output_dir / f"bundled_web_shopping_{tag}_ai_annotations.json"
...
def parse_args():
    ...
    parser.add_argument("--category", default=None,
                        help="Filter by category, e.g. baking_item_0 (default: all)")
