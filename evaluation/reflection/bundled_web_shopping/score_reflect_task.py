...
Computes Reflection Task metrics from the output of run_reflect_task.py for BundledWebShopping.
...
LLM_JUDGE_SYSTEM = """You are a strict evaluator. You will be shown two one-sentence descriptions of an agent's failure on a BundledWebShopping multi-subtask web shopping task. Determine whether they describe the same underlying failure. Minor differences in wording are acceptable; what matters is whether the core failure being described is the same.
...
def score_q2(results: list[dict]) -> dict:
    ...
    for r in results:
        gt_ranges = r.get("_gt_core_ranges")
