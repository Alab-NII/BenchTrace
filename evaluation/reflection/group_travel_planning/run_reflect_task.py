...
Runs the Reflection Task evaluation for a given model on GroupTravelPlanning snapshots.
...
FINAL_DATASET_DIR = Path(__file__).resolve().parent.parent.parent / "final_dataset" / "group_travel_planning"

ALL_GAMES = ["5_travelers", "6_travelers", "7_travelers", "8_travelers"]
...
def format_trajectory(trajectory: list[dict]) -> str:
    ...
    for step in trajectory:
        ...
        action = _truncate(step.get("action") or "", MAX_ACTION_CHARS)
        progress = step.get("progress", 0.0)
        sub_prog = step.get("subtask_progress")
        subtask = step.get("subtask_idx")
        meta = f"subtask={subtask}"
        if sub_prog is not None:
            meta += f" sub_progress={sub_prog}"
        if not action:
            lines.append(f"[Step {step['step']}] ({meta}) Obs: {obs} | Progress: {progress}")
        else:
            lines.append(f"[Step {step['step']}] ({meta}) Obs: {obs} | Action: {action} | Progress: {progress}")
    return "\n".join(lines)
...
def run(args):
    ...
    system_prompt = load_prompt("system_group_travel_planning.md")
