...
Runs the Reflection Task evaluation for a given model.
For each snapshot in the final dataset, queries the model with 3 questions:
...
FINAL_DATASET_DIR = Path(__file__).resolve().parent.parent.parent / "final_dataset" / "jericho"

ALL_GAMES = ["balances", "detective", "library", "temple", "zork1", "zork3"]
...
def format_trajectory(trajectory: list[dict]) -> str:
    ...
    for step in trajectory:
        ...
        inv = step.get("inv", "").replace("\n", " ").strip()
        lines.append(f"[Step {step['step']}] Obs: {obs} | Action: {action} | Inv: {inv}")
    return "\n".join(lines)
...
def parse_q1(raw: str) -> str | None:
    ...
    if isinstance(parsed, dict):
        ...
        if ans in ("yes", "no"):
            return ans
    # Fallback: search for yes/no in raw text
    lower = raw.lower()
...
def evaluate_snapshot(ep: dict, api: str, model: str, system_prompt: str,
    ...
    common_kwargs = dict(game=game, final_score=final_score, max_score=max_score, trajectory=traj_str)
...
def load_episodes(games: list[str]) -> list[dict]:
    episodes = []
    for game in games:
...
def retry_episode(r: dict, episodes_by_id: dict, api: str, model: str,
    ...
    snapshot = ep["snapshot"]
    traj_str = format_trajectory(snapshot["trajectory"])
    common_kwargs = dict(game=ep["game"], final_score=snapshot["final_score"],
                         max_score=snapshot["max_score"], trajectory=traj_str)
...
def run(args):
    ...
    system_prompt = load_prompt("system_jericho.md")
