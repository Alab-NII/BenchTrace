...
SYSTEM_PROMPT = """You are evaluating whether a model's diagnosis of an agent's failure in a text adventure game matches the ground truth diagnosis.

You will be given:
1. A trajectory excerpt showing the agent's actions and observations around the failure
...
def format_excerpt(trajectory: list[dict], step_start: int, step_end: int, window: int) -> str:
    ...
    hi = min(len(trajectory) - 1, step_end + window)
    lines = []
    for step in trajectory:
        ...
        if lo <= s <= hi:
            ...
            lines.append(f"[Step {s}] Obs: {obs} | Action: {action}{marker}")
    return "\n".join(lines)
...
def run(args):
    ...
    # Collect all Q3 items that need judging
    items = []  # (result_idx, q3_idx, episode_id, where, gt, pred)
