...
LLM-as-judge for Q3 (Diagnosis) on ScienceWorld snapshots.
...
FINAL_DATASET_DIR = Path(__file__).resolve().parent.parent.parent / "final_dataset" / "scienceworld"

SYSTEM_PROMPT = """You are evaluating whether a model's diagnosis of an agent's failure on a ScienceWorld scientific reasoning task matches the ground truth diagnosis.
    ...
    for step in trajectory:
        ...
        if lo <= s <= hi:
            ...
            if s == 0 or not action:
                lines.append(f"[Step {s}] Obs: {obs} | Progress: {progress}{marker}")
            else:
                lines.append(f"[Step {s}] Obs: {obs} | Action: {action} | Progress: {progress}{marker}")
    return "\n".join(lines)
