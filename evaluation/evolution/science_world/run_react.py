#!/usr/bin/env python3
"""
ScienceWorld Evolution Evaluation runner — ReAct baseline
...
ScienceWorld differences from AlfWorld/BabyAI:
  - Actions are free-form text strings (like Jericho); env.step() takes string directly
  - task_name and variation parsed from episode ID via regex (no cache needed)
...
REACT_SYSTEM_PROMPT = (
    ...
    "At each step you will be given the current observation describing what you see. "
    "Respond with a Thought (one sentence of reasoning) "
    "followed by an Act (a free-form text command). "
    "Do NOT repeat a failed action. Try different approaches when stuck."
)

REACT_FORMAT = (
    "\nValid action types include: look around, pick up OBJ, move OBJ to CONTAINER, "
    "heat OBJ with OBJ, mix OBJ, measure OBJ with OBJ, read OBJ, examine OBJ, "
    "go to ROOM, open OBJ, close OBJ, and similar natural language commands.\n"
