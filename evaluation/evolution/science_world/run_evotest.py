#!/usr/bin/env python3
"""
ScienceWorld Evolution Evaluation runner — EvoTest baseline
...
ScienceWorld differences from AlfWorld:
  - env.step(action_str) takes free-form text directly (no index conversion)
  - task_name and variation parsed from episode ID
  - Positive memory uses info["won"] instead of score-up obs pattern
  - Negative memory uses repeated-action loop detection
...
INITIAL_GUIDING_PROMPT = "Explore the environment systematically and conduct experiments carefully to complete the science task."
...
def evolve(cur_prompt: str, cur_code: str, game_history_str: str,
    ...
    sys_prompt = (
        "You are an expert at science experiments in text-based environments. Your goal is to "
        "analyze the existing prompt, state extractor code, and episode history, and generate "
        "a better prompt and state extractor code that will help an LLM agent complete more experiments."
    )

    user_prompt = f'''
Generate a new improved guiding prompt and state extractor code for a science experiment agent.
...
1. Identify useful actions that led to experiment progress. Give step-by-step instructions.
...
def _extract_positive_from_traj(trajectory: list[dict]) -> list[dict]:
    """If the episode was won, the step before done is a positive example."""
    positives = []
