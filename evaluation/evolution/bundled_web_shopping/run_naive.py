#!/usr/bin/env python3
"""
BundledWebShopping Evolution Evaluation runner — Naive Concatenation baseline

Concatenates all evolution snapshot trajectories (subtask Q + selection +
correct/incorrect) before the test trajectory's pre-decision-point context,
then runs a rolling-scratchpad ReAct agent from the failing subtask onwards.

BundledWebShopping differences from step-based environments (AlfWorld):
  - One env.step() = one complete subtask (product selection)
  - decision_point is even; decision_point//2 = failing subtask index
  - Trajectory uses paired steps (even = pre-action obs, odd = post-action)
  - We summarize each evolution episode as a list of subtask Q + selection
    pairs, which is far more compact than full step-by-step replay
"""
...
# ---------------------------------------------------------------------------
# Trajectory formatting (subtask-level, like format_trajectory_for_evo but
# slightly more verbose so the agent can compare option attributes)
# ---------------------------------------------------------------------------

def _format_evolution_trajectory(ep_id: str, trajectory: list[dict],
                                  final_score: float) -> str:
    lines = [f"=== Past Episode (id: {ep_id}, final_score: {final_score:.2f}) ==="]
    for step in trajectory:
        ...
        status = "CORRECT" if correct else "INCORRECT" if correct is False else "?"
        lines.append(f"\n[Subtask {idx + 1}] Question:\n{obs[:400]}")
...
class NaiveConcatBaseline:
    """Concatenate all evolution episode summaries as a fixed primer."""
    ...
    def build_primer(
        self,
        evolution_episodes: list[tuple[str, dict]],
        pre_dp_context: str,
    ) -> str:
        parts = []
        for ep_id, ep in evolution_episodes:
            traj = ep["snapshot"]["trajectory"]
            final_score = float(ep["snapshot"].get("final_score", 0) or 0)
            parts.append(_format_evolution_trajectory(ep_id, traj, final_score))

        if pre_dp_context:
            parts.append(
                "=== Current Episode — Prior Subtask Selections (before decision point) ===\n"
                + pre_dp_context
            )
...
def run_task(task: dict, snapshots: dict, baseline: NaiveConcatBaseline,
    ...
    evolution_episodes = [
        (ep_id, snapshots[ep_id]) for ep_id in evolution_ep_ids
        if ep_id in snapshots
    ]
    primer = baseline.build_primer(evolution_episodes, pre_dp_context)
...
def main():
    ...
    with open(results_path, "w") as f:
        json.dump({
            "game": args.game, "baseline": "naive_concat",
