#!/usr/bin/env python3
"""
Evolution Evaluation runner - Naive Concatenation baseline

Concatenates all evolution snapshot trajectories before the test trajectory
(truncated to decision_point), then runs a rolling-scratchpad ReAct-style
agent from the decision point onwards.
"""
...
from utils import (
    ...
    load_snapshots, restore_game_state, run_episode_from_decision_point,
    format_trajectory, parse_action,
...
class NaiveConcatBaseline:
    """
    Concatenate all evolution snapshot trajectories as a primer, plus a rolling
    scratchpad of post-dp (obs, inv, action) accumulating each step.
    Primer is truncated from the beginning if too long.
    """
    ...
    def build_primer(
        ...
        test_trajectory: list[dict],
    ) -> str:
        """Format evolution histories + pre-dp test trajectory as a fixed primer."""
        parts = []
        ...
        # pre-dp test trajectory in scratchpad format
        pre_dp_lines = []
        for s in test_trajectory:
            pre_dp_lines.append(f"Obs {s['step']}: {s['obs']}")
            if s.get("inv"):
                pre_dp_lines.append(f"Inv {s['step']}: {s['inv']}")
            pre_dp_lines.append(f"Act {s['step']}: {s['action']}")
        parts.append("=== Current Episode History (before decision point) ===\n" + "\n".join(pre_dp_lines))
        full_primer = "\n\n".join(parts)
        return truncate_text(full_primer, MAX_CONTEXT_TOKENS)

    def make_agent_fn(self, primer: str):
        scratchpad = []  # grows with each post-dp step: Obs / Inv / Act
        ...
        def agent_fn(obs: str, inv: str, step: int):
            ...
            user_prompt += "\n".join(scratchpad)
            user_prompt += ACTION_FORMAT_INSTRUCTION
...
def run_task(task: dict, snapshots: dict, baseline: NaiveConcatBaseline,
    ...
    primer = baseline.build_primer(evolution_trajs, truncated_test_traj)
