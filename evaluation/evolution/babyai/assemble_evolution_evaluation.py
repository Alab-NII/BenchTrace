#!/usr/bin/env python3
"""
Assemble Evolution Evaluation dataset for BabyAI.

Mirrors ROGUE/final_dataset/assemble_evolution_evaluation.py (Jericho),
adapted for BabyAI:
  - "game" = "level-N" (e.g., "level-1")
  - Snapshot structure uses progress (0-1) for final_score / max_score=1.0
  - No ROM dir; env identified by level_id + instance_seed (not stored in final dataset)

3D task matrix:
  Type 1: same level, same failure type, similar where[0]  (≤15 steps)
  Type 2: same level, same failure type, different instance (>15 steps)
  Type 3: different level, same failure type
  Distance: 1, 2, 3, 5, 10  (number of evolution snapshots shown)
  FailureType: one cell per meaningful failure type

Output: final_dataset/babyai/{game}/evolution_evaluation.json
"""
...
DATASET_ROOT = Path(__file__).parent.parent.parent / "final_dataset" / "babyai"
...
def main():
    ...
    for game, eps in sorted(all_snapshots.items(), key=lambda x: int(x[0].split('-')[1])):
    ...
    print("\n=== Results ===")
    total = 0
    for game in sorted(all_snapshots.keys(), key=lambda x: int(x.split('-')[1])):
        tasks = results[game]
        total += len(tasks)
        ...
        for k in sorted(cell_counts[game]):
            print(
                ...
                f"{k[2]:40s}: {cell_counts[game][k]}"
            )

    print(f"\nTotal: {total} tasks across {len(all_snapshots)} levels")
