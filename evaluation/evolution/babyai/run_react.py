#!/usr/bin/env python3
"""
BabyAI Evolution Evaluation runner — ReAct baseline
...
BabyAI differences from Jericho/AlfWorld:
  - Actions constrained to 7 discrete strings (shown to agent explicitly)
  - env.step() takes integer index; LLM outputs string → action_str_to_idx()
  - env created with BabyAIEnv(level_id, obs_to_reward, seed=instance_seed)
  - instance_seed and level_id looked up from seed cache at runtime
...
REACT_SYSTEM_PROMPT = (
    ...
    "followed by an Act (one of the valid actions). "
...
def parse_react_response(response: str) -> tuple[str, str]:
    from src.babyai_env import ACTIONS
    thought = ""
    action = "move forward"
    if not response:
...
def run_task(task: dict, snapshots: dict, baseline: ReActBaseline,
    ...
    # Create env for this specific episode
