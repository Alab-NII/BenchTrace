#!/usr/bin/env python3
"""
GroupTravelPlanning Evolution Evaluation runner — AutoSkill baseline

AutoSkill: experience-driven lifelong learning via skill self-evolution.
Each past evolution snapshot → LLM-extracted planning skill. At inference,
top-k skills retrieved by cosine similarity to the current traveler's request.
"""
...
SKILL_EXTRACTION_SYSTEM = (
    ...
    "Extract a concise, reusable planning skill from a past episode."
)

SKILL_EXTRACTION_TEMPLATE = """\
Below is a trajectory from a group travel planning episode.
Final score: {final_score:.2f} (1.0 = all constraints satisfied)

{trajectory_text}

Extract ONE reusable skill — a generalizable planning strategy learned from this episode.
Focus on WHAT the planner should do (or avoid) when faced with similar traveler constraints,
especially around cuisine types, price ranges, ratings, and JOIN/RELATION cross-traveler refs.

Respond in EXACTLY this format (no extra text):
name: [2-5 word kebab-case name]
description: [1 sentence: what travel planning scenario this skill applies to]
instructions: [2-3 sentences: what planning strategy to use]
...
def _format_traj_for_extraction(trajectory: list[dict]) -> str:
    ...
    for step in trajectory:
        ...
        lines.append(f"[Traveler {idx+1}] Request: {obs[:80]} → {prog:.0%}")
    return "\n".join(lines)
...
def render_skills(skills: list[Skill], max_chars: int = 1500) -> str:
    if not skills:
        return ""
    lines = ["=== Relevant Planning Skills from Past Episodes ==="]
    for s in skills:
        block = (f"\n[{s.name}]\n"
